...
function createPositionsManager({
  ...
  const POSITIONS_ERROR_AUTO_HIDE_MS = 5_000;
  let positionsErrorHideTimer = null;

  const quoteByStreamerSymbol = new Map();
  const quoteSubscriptions = new Set();
  const plRowsByStreamerSymbol = new Map();

  function clearPositionsErrorTimer() {
    if (!positionsErrorHideTimer) return;

    clearTimeout(positionsErrorHideTimer);
    positionsErrorHideTimer = null;
  }

  function clearStatusMessage() {
    clearPositionsErrorTimer();
  ...
  function setErrorMessage(message) {
    ...
    positionsErrorHideTimer = setTimeout(() => {
      setStatus(positionsStatus, '', '');
      positionsErrorHideTimer = null;
    }, POSITIONS_ERROR_AUTO_HIDE_MS);
  }

  function setSectionVisible(isVisible) {
    if (!positionsSection) return;
    positionsSection.classList.toggle('hidden', !isVisible);
  }
  ...
  function computeUnrealizedPositionPl(position, quote) {
    ...
    const isLong = quantityDirection === 'long';
    const isShort = quantityDirection === 'short';
    if (!isLong && !isShort) return null;

    const averageOpenPrice = parseDecimal(position?.averageOpenPrice);
    const quantity = parseDecimal(position?.quantity);
    const multiplier = parseDecimal(position?.multiplier);
    if (averageOpenPrice === null || quantity === null || multiplier === null) return null;
    ...
    let mark = null;
    if (isLong) {
      mark = hasBid ? bidPrice : (hasBid || hasAsk ? ((bidPrice ?? askPrice) + (askPrice ?? bidPrice)) / 2 : null);
    } else {
      mark = hasAsk ? askPrice : (hasBid || hasAsk ? ((bidPrice ?? askPrice) + (askPrice ?? bidPrice)) / 2 : null);
    }

    if (mark === null || !Number.isFinite(mark)) return null;

    return isLong
      ? (mark - averageOpenPrice) * quantity * multiplier
      : (averageOpenPrice - mark) * quantity * multiplier;
  }
    ...
    try {
      ...
      if (symbolsToRemove.length > 0) {
        sendDxLinkMessage({
          ...
          channel: feedChannel,
          remove: symbolsToRemove.map(symbol => ({ type: 'Quote', symbol })),
        });
      ...
      if (symbolsToAdd.length > 0) {
        sendDxLinkMessage({
          type: 'FEED_SUBSCRIPTION',
          channel: feedChannel,
          add: symbolsToAdd.map(symbol => ({ type: 'Quote', symbol })),
        });
  ...
  async function closePosition(position, buttonEl = null) {
    ...
    try {
      await apiPost(`/accounts/${accountNumber}/orders`, order);
      clearStatusMessage();
      await loadPositions();
    } catch (err) {
  ...
  function handleQuote(eventSymbol, bidPrice, askPrice) {
  ...
  function reapplySubscriptions() {
    if (quoteSubscriptions.size === 0) return;

    const positionSymbols = Array.from(quoteSubscriptions).map(symbol => ({ type: 'Quote', symbol }));
    sendDxLinkMessage({
      type: 'FEED_SUBSCRIPTION',
      channel: feedChannel,
      add: positionSymbols,
