...
function createPositionsManager({
  ...
  function computeUnrealizedPositionPl(position, quote) {
    ...
    if (contractCount === null || contractCount <= 0 || multiplier === null) return null;
    ...
    const averageOpenPrice = useSandbox
      ? quoteMid
      : parseDecimal(position?.averageOpenPrice);
    if (averageOpenPrice === null || !Number.isFinite(averageOpenPrice)) return null;

    let mark = null;
    if (isLong) {
      mark = hasBid ? bidPrice : quoteMid;
    } else {
      mark = hasAsk ? askPrice : quoteMid;
    }

    if (mark === null || !Number.isFinite(mark)) return null;

    return isLong
      ? (mark - averageOpenPrice) * contractCount * multiplier
      : (averageOpenPrice - mark) * contractCount * multiplier;
  }
  ...
  function reapplySubscriptions() {
    ...
    addQuoteSubscriptions?.(Array.from(quoteSubscriptions));
  }
