...
function createTastyTradeMarketDataManager({
  controlChannel,
  feedChannel,
  ...
  async function resolveStreamerSymbol(symbol) {
    ...
    const instrument = await getEquityInstrument?.(symbol);
...
window.createTastyTradeMarketDataManager = createTastyTradeMarketDataManager;
