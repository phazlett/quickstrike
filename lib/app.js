...
const DEFAULT_ORDER_STATUS_MESSAGE = 'No in-flight orders';
const MAX_ORDER_STATUS_LOG_ENTRIES = 10;
...
function setOrderStatus(message, type) {
  ...
  const shouldShowInlineStatus = normalizedType === 'info'
    && orderSubmitInFlight
    && !!normalizedMessage;

  if (execStatusToolbarEl && execStatusMessageEl) {
    execStatusToolbarEl.classList.remove('hidden');
    execStatusToolbarEl.classList.toggle('orders-status-active', shouldShowInlineStatus);
    execStatusToolbarEl.classList.remove('orders-status-error');

    if (!shouldShowInlineStatus) {
      execStatusMessageEl.textContent = DEFAULT_ORDER_STATUS_MESSAGE;
      ...
      orderStatusLogEntries.push(normalizedMessage);
      if (orderStatusLogEntries.length > MAX_ORDER_STATUS_LOG_ENTRIES) {
        orderStatusLogEntries = orderStatusLogEntries.slice(-MAX_ORDER_STATUS_LOG_ENTRIES);
      }
      execStatusMessageEl.textContent = orderStatusLogEntries
        .map((entry, index) => `${index + 1}. ${entry}`)
        .join('\n');
      if (execStatusCancelBtn) {
        if (shouldShowInlineStatus && IS_OAUTH_ADAPTER) {
...
function extractOrderIdFromSubmitResponse(response) {
...
function extractOrderStatusText(order) {
...
function getTerminalOrderStatus(statusText) {
  const normalized = normalizeUppercaseString(statusText);
  ...
  const match = Object.entries(statusMap).find(([fragment]) => normalized.includes(fragment));
  return match ? match[1] : '';
}

function extractOrderRejectReason(order) {
  return normalizeString(
    order?.['reject-reason']
...
function isRejectedOrderStatus(statusText) {
  const normalized = normalizeUppercaseString(statusText);
  return ['REJECT', 'FAILED', 'INVALID', 'ERROR', 'CANCEL REJECT'].some(fragment => normalized.includes(fragment));
}
...
function findSubmittedOrder(orders, { orderId, symbol, action }) {
  if (!Array.isArray(orders) || orders.length === 0) return null;

  if (orderId) {
    const exactMatch = orders.find(order => getOrderId(order) === orderId);
  ...
  candidates.sort((a, b) => getOrderSortTimestamp(b) - getOrderSortTimestamp(a));
  return candidates[0];
}

async function waitForSubmittedOrderTerminalFailure({
  accountNumber,
  orderId,
  symbol,
  action,
  ...
  if (typeof brokerAdapter?.listOrders !== 'function') return '';

  const startedAt = Date.now();
  while (Date.now() - startedAt <= timeoutMs) {
    assertOrderAutomationNotCancelled();

    const response = await brokerAdapter.listOrders(accountNumber);
    const orders = Array.isArray(response?.data?.items) ? response.data.items : [];
    const submittedOrder = findSubmittedOrder(orders, { orderId, symbol, action });

    if (submittedOrder) {
      const statusText = extractOrderStatusText(submittedOrder);
      const terminalStatus = getTerminalOrderStatus(statusText);

      if (terminalStatus && terminalStatus !== 'FILLED') {
        return extractOrderRejectReason(submittedOrder) || `Order ended with status ${terminalStatus}.`;
      }
...
function extractTastytradeSubmitErrorMessage(response) {
  const data = response?.data ?? {};
  const order = data?.order ?? data?.['order'] ?? {};
  const normalizedStatus = normalizeUppercaseString(
    order?.status
    ?? order?.['status']
    ?? order?.['order-status']
    ?? order?.['status-description']
  ...
  const explicitErrorMessage = normalizeString(
  ...
  const errorItems = [];
  const appendErrorItems = source => {
    if (!Array.isArray(source)) return;
    source.forEach(item => {
      const message = normalizeString(item?.message ?? item?.['message']);
      if (message) errorItems.push(message);
    });
  };

  appendErrorItems(response?.error?.errors);
  appendErrorItems(data?.error?.errors);
  appendErrorItems(data?.errors);

  if (errorItems.length > 0) {
    return errorItems.join(', ');
  }

  const rejectReason = normalizeString(
    order?.['reject-reason']
    ?? order?.['rejection-reason']
    ?? order?.['reject_reason']
    ?? order?.['status-description']
  ...
  if (isRejected) {
    return rejectReason || 'Order rejected by broker.';
  }

  const terminalStatus = getTerminalOrderStatus(normalizedStatus);
  if (terminalStatus && terminalStatus !== 'FILLED') {
    return rejectReason || `Order ended with status ${terminalStatus}.`;
  }
  ...
  const statusIndicatesFailure = [
    ...
    'REJECT',
    'REMOVED',
  ].some(fragment => normalizedStatus.includes(fragment));

  if (statusIndicatesFailure) {
    return rejectReason || `Order not accepted by broker (${normalizedStatus || 'UNKNOWN'}).`;
  }
...
function getMatchingLiveOrdersForIntent(orders, { symbol, action }) {
  ...
  const isTerminalOrderStatus = order => {
    const status = extractOrderStatusText(order);
    const terminalStatus = getTerminalOrderStatus(status);
    ...
    return [
      ...
      'DONE',
      'COMPLETED',
    ].some(terminal => normalizedStatus.includes(terminal));
  };
...
function isLikelyUsOptionsMarketClosedNow() {
  const now = new Date();
  ...
  const weekday = normalizeUppercaseString(parts.find(part => part.type === 'weekday')?.value);
  const hour = Number.parseInt(parts.find(part => part.type === 'hour')?.value ?? '', 10);
  const minute = Number.parseInt(parts.find(part => part.type === 'minute')?.value ?? '', 10);

  if (!Number.isFinite(hour) || !Number.isFinite(minute) || !weekday) return false;
...
async function submitOpeningOrderWithLimitChase({ accountNumber, symbol, action, quantity }) {
  ...
  if (isLikelyUsOptionsMarketClosedNow()) {
    throw new Error('Market is currently closed. Order automation is paused outside regular trading hours.');
  }
  ...
  for (let attempt = 1; attempt <= executionSettings.attempts; attempt += 1) {
    ...
    const submitResponse = await brokerAdapter.submitOrder(accountNumber, limitOrder);
    const submitErrorMessage = extractTastytradeSubmitErrorMessage(submitResponse);
    if (submitErrorMessage) {
      throw new Error(submitErrorMessage);
    }

    const submittedOrderId = extractOrderIdFromSubmitResponse(submitResponse);
    try {
      const postSubmitRejectedMessage = await waitForSubmittedOrderTerminalFailure({
        accountNumber,
        orderId: submittedOrderId,
        symbol,
        action,
        timeoutMs: Math.min(4_000, Math.max(1_000, executionSettings.fillTimeoutMs)),
        pollIntervalMs: executionSettings.pollIntervalMs,
      });
      if (postSubmitRejectedMessage) {
        throw new Error(postSubmitRejectedMessage);
      }
    } catch (error) {
      const message = normalizeString(error?.message).toLowerCase();
    ...
    if (isLikelyUsOptionsMarketClosedNow()) {
      await cancelMatchingLiveOrdersForIntent({ accountNumber, symbol, action, executionSettings });
      throw new Error('Market is currently closed. Stopped order automation instead of repricing.');
    }
